...
void re_phai() {
  ...
  // Xoay phải
  int turnSpeed = 150;
  ledcWrite(PWM_PIN_L_A, turnSpeed);
  ledcWrite(PWM_PIN_L_B, 0);
  ledcWrite(PWM_PIN_R_A, 0);
  ledcWrite(PWM_PIN_R_B, turnSpeed);
  ...
  while (millis() - startTime < 1000) {
    int s2_check = (analogRead(SENSOR_2_PIN) <= threshold[1]) ? 1 : 0;
    int s3_check = (analogRead(SENSOR_3_PIN) <= threshold[2]) ? 1 : 0;
